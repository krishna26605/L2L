...
export const AuthProvider = ({ children }) => {
  ...
  const signIn = async (email, password) => {
    try {
      ...
      console.log('✅ useAuth: Login successful - full response:', response);
      
      // Note: authAPI.login already stores tokens in cookies
      if (response.data) {
        setUser(response.data.user);
        console.log('👤 User set in context:', response.data.user);
        // Return the full response data including redirectTo
  ...
  const signUp = async (email, password, displayName, role) => {
    try {
      ...
      console.log('✅ useAuth: Registration successful - full response:', response);
